...
export async function middleware(request: NextRequest) {
  ...
  // Path-based tenant resolution for /school/[slug]
  const schoolMatch = pathname.match(/^\/school\/([^\/]+)/);
  const isBaseSchoolPage = /^\/school\/[^\/]+$/.test(pathname);
  if (schoolMatch && !tenantContext) {
    const slug = schoolMatch[1];
    tenantContext = await resolveTenantFromSlug(slug, "school-suite");
  }
