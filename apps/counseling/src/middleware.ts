...
export async function middleware(request: NextRequest) {
  ...
  // Path-based tenant resolution for /counseling/[slug]
  const counselingMatch = pathname.match(/^\/counseling\/([^\/]+)/);
  const isBaseCounselingPage = /^\/counseling\/[^\/]+$/.test(pathname);
  if (counselingMatch && !tenantContext) {
    const slug = counselingMatch[1];
    tenantContext = await resolveTenantFromSlug(slug, "counseling");
  }
